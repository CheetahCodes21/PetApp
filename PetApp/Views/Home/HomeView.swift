...
struct HomeView: View {
    ...
    var body: some View {
        ...
        // KAN-20: the Record button opens the capture + save flow, seeded with
        // the current daily prompt. `.recordingRecovery` offers back a recording
        // interrupted by termination on the next launch. The save itself is
        // persisted by the save sheet, so no extra work is needed here.
        .memoryRecorder(isPresented: $showRecording,
                        question: DailyPrompts.all[promptIndex])
        .recordingRecovery()
    }
